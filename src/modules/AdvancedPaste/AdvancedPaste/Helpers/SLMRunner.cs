...
namespace AdvancedPaste.Helpers
{
    ...
    {
        // private readonly string _modelDir = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "onnx-models", "phi3-directml-int4-awq-block-128");
        private readonly string _modelDir = "C:\\temp\\phi3";
        ...
        public async IAsyncEnumerable<string> InferStreamingAsync(string prompt, [EnumeratorCancellation] CancellationToken ct = default)
        {
            ...
            while (!generator.IsDone())
            {
                ...
                try
                {
                    ...
                    {
                        Console.WriteLine($"Tokens per second: {tokenCount / stopwatch.Elapsed.TotalSeconds}");
                        stopwatch.Restart();
                        tokenCount = 0;
                    }
